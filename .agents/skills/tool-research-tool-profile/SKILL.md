---
name: tool-research-tool-profile
description: Use when a single tool needs a deep profile. Research one tool at a time across features, hosting model, enterprise readiness, pricing, and fit so the results can be compared thoroughly against other tools.
---

# Tool Research Tool Profile

## Overview

Use this skill to produce a deep profile for one tool only. This is the unit of work that should be repeated for the candidate tool, the tools already in use, and the most relevant competitors as part of a sequential workflow.

## Workflow

1. Confirm which single tool is in scope and why it matters to the evaluation.
2. Research the tool using the framework in `references/tool-profile-framework.md`.
3. Capture facts first, then clearly labeled inference where facts are incomplete.
4. Summarize where the tool fits well, where it does not, and what would matter in a side-by-side comparison.
5. Produce one profile page that can be linked from the final report.
6. Mark the tool-profile task as complete in the task list.
7. Hand off to the next tool profile or to synthesis only after this profile is complete.

## Required Output

Produce one tool profile with these sections:

- Tool summary
- Target users and primary use cases
- Feature profile
- Hosting and deployment model
- Enterprise readiness
- Pricing and licensing
- Strengths
- Weaknesses
- Best-fit scenarios
- Risks and open questions

## Working Rules

- Keep the profile focused on one tool only.
- Finish the current tool before starting the next one.
- Include whether the tool is cloud-only, local-first, self-hosted, or hybrid.
- Include whether the tool offers enterprise licensing, SSO, SCIM, admin controls, or procurement-friendly plans when relevant.
- Include pricing facts when available and clearly labeled assumptions when they are not.
- Write the profile so it can be compared directly to other tool profiles.

## Handoff

Pass completed tool profiles to `tool-research-analysis` for synthesis across tools.

## Resources

- Read `references/tool-profile-framework.md` before profiling a tool.
